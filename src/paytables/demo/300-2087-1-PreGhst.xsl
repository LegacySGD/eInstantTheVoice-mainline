			...
			<lxslt:component prefix="my-ext" functions="formatJson">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, convertedPrizeValues, prizeNamesDesc)
					{
						...
						r.push('<td class="tablehead">');
						r.push(getTranslationByName("stepsRevealed", translations));
						r.push('</td>');
						
						r.push('<td class="tablehead">');
						r.push(getTranslationByName("stepsTotal", translations));
						r.push('</td>');
						...
						for(var prize = 0; prize < prizeNamesList.length; ++prize)
						{
							...
							if(isNaN(prizeNamesList[prize]))
							{
								...
								registerDebugText(prizeNamesList[prize] + " collection count: " + numCollected);
								r.push('<td class="tablebody">');
								r.push(numCollected);
								r.push('</td>');

								r.push('<td class="tablebody">');
								r.push(collectionsList[prize]);
								r.push('</td>');
